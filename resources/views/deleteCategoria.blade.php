...
<div class="bg-slate-600 rounded p-32 text-white container mt-6 mx-auto justify-content-around">
    <h1 class="text-5xl">Borrar categoria</h1>
<form action={{route('borrarCatego',$item->id)}} method="POST" class="w-full">
    @csrf
    @method('DELETE')
    <div class="flex items-center border-b border-teal-500 py-2">
        <label  class="text-3xl" for="nombre">Nombre</label>
        <input id="nombre" name="nombre" class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" value={{$item->nombre}} readonly>
    </div>
